...
class ComputeCurvature:
    ...
    def get_curve_plotting_values(self, obj): 
        theta_fit = np.linspace(-np.pi, np.pi, 180)
        x_fit = obj.xc + obj.r*np.cos(theta_fit)
        y_fit = obj.yc + obj.r*np.sin(theta_fit)
        return x_fit, y_fit

    def curve_plot(self, curvature, x, y, x_fit, y_fit, name, initial):
        addr_to_save = './outputs' 
        addr = os.path.join(addr_to_save, f'{initial}_results_{name}.jpg')
        plt.figure()
