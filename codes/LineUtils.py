...
class LineUtils(object):
    ...
    def get_mm(self, point1, point2): 
        pixels = math.sqrt(abs(int(point1[0]) - int(point2[0]))**2 + abs(int(point1[1]) - int(point2[1]))**2)
        mm = round((pixels * 25.4) / 720, 2) 
        return mm

    def image_preprocessing(self, image, central, is_ouside=True, card_type=None):
        
        img_ori = image.copy()

        cnts, top, bottom, right, left, \
            one_top, two_bottom, one_right, two_left, \
            top_line, bottom_line, right_line, left_line, central = self.get_outermost_line_to_measure(image, central, is_ouside=is_ouside, card_type=card_type)
        
        # Select the card only
        gap = 0
        select = image[top_line[1] - gap:bottom_line[1] + gap, left_line[0] - gap:right_line[0] + gap]

        # reshape the card
        reshaped_card = self.maintain_aspect_ratio_resize(select, width=1830) # default size width: 1830

        #  Put the image into a new frame
        new_img = 255 * np.ones(img_ori.shape, np.uint8)
        a, b = (new_img.shape[0] - reshaped_card.shape[0]) // 2, ((new_img.shape[0] - reshaped_card.shape[0]) // 2) + reshaped_card.shape[0]
        c, d = (new_img.shape[1] - reshaped_card.shape[1]) // 2, ((new_img.shape[1] - reshaped_card.shape[1]) // 2) + reshaped_card.shape[1]
        new_img[a:b, c:d] = reshaped_card

        # print('#############', a, b, c, d)
        # print('#############', img_ori.shape, new_img.shape, reshaped_card.shape)
