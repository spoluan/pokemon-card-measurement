...
from codes.Curvature import ComputeCurvature
...
select = image[top_line[1] - 10:bottom_line[1] + 10, left_line[0] - 7:right_line[0] + 7]

# reshape the card
reshaped_card = lineUtils.maintain_aspect_ratio_resize(select, width=1830)

#  Put the image into a new frame
new_img = 255 * np.ones(image.shape, np.uint8)
...
cv2.imwrite(os.path.join(addr_to_save, img_path), reshaped_card) 
# cv2.imwrite(os.path.join(addr_to_save, img_path), new_img)
# cv2.imwrite(os.path.join(addr_to_save, img_path), select)
# select.shape
# reshaped_card.shape

# mm_tb = lineUtils.get_mm(top_line, bottom_line)
# mmlr  = lineUtils.get_mm(right_line, left_line)
